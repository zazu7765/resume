...
\begin{customS}
    ...
    {Toronto, Canada}
\item Responsible for everything lol
\end{customS}
\begin{customS}
    {Robotics For All}
...
\begin{customS}
    ...
    {Robotics For All}
    {}
\item Developed a Slack application/integration to simplify the substitution process for teachers
\item Utilized a Google Sheet and Cloud API to retrieve available volunteer teachers and coordinators
\item Coordinators can make class requests in a Slack dashboard with specific details, which are stored in a PostgreSQL database for further data analysis.
\item Teachers can use the Slack dashboard to accept requests and contact coordinators for more information
\item All management can be done within Slack, without the need for further configuration
\item Deployed using AWS ECR/ECS on an EC2 instance connected to an RDS database.
\end{customS}
\begin{customS}
    {Simple Notes}
    {Go, Postgres, Docker}
